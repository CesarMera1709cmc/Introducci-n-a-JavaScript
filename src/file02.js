...
frameworks.forEach(framework => {
  const parts = framework.split('|').map(item => item.trim());
  
  // Verificación adicional para asegurar que tenemos los 4 elementos
  if (parts.length === 4) {
    const [frameworkName, releaseDate, usersName, popularityPercentage] = parts;
    ...
    console.warn(`Formato incorrecto para el framework: ${framework}`);
  }
});